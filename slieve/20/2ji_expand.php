...
for ($i = 0; $i < 20; $i += 2) {
    ...
    if($le[$i+1] >= 0){$lef2 = "+".$le[$i+1];}else{$lef2 = $le[$i+1];}
    if($ri[$i+1] >= 0){$rig2 = "+".$ri[$i+1];}else{$rig2 = $ri[$i+1];}
    $pdf->Cell(0, 30,"(". ($i + 2) .  ")  " . "(x".$lef2.")(x".$rig2.")");
    $pdf->Ln(23);
}
...
$sql = "INSERT INTO elementanswerlist(
	...
	'$date', '$Id' , '$leftlist','$rightlist','$anserlist','2012'
)";
