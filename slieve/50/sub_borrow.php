...
for($i = 45;$i < 50;$i++){
    do {
        do {
            $left[$i] = random_int(10, 20);
            $right[$i] = random_int(0, 9);
        } while ($left[$i] - $right[$i] >= 10);
    } while ($left[$i] - $right[$i] < 0);
    $ans[$count] = $left[$i] - $right[$i];
    $count++;
}
...
$pdf->Cell(90, 20, 'つぎのもんだいにこたえましょう');
$pdf->Ln(20);
$pdf->SetFontSize(20); //フォントサイズ変更。
// $siki 配列の長さを取得
...
$sql = "INSERT INTO elementanswerlist(
	...
	'$date', '$Id' , '$leftlist','$rightlist','$anserlist','5004'
)";
