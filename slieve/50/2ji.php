...
// $ex_ans 配列の長さを取得
$ex_ans_length = count($ex_ans);

for ($i = 0; $i < $ex_ans_length; $i += 3) {
    $pdf->Cell(60, 20,"(". ($i + 1) . ")  " . $ex_ans[$i]);
    $pdf->Cell(60, 20, "(". ($i + 2) .  ")  " . $ex_ans[$i + 1]);
    
    // $i + 2 が配列の範囲内であるか確認
    if ($i + 2 < $ex_ans_length) {
        $pdf->Cell(0, 20, "(". ($i + 3) . ")  " . $ex_ans[$i + 2]);
    }
...
$sql = "INSERT INTO elementanswerlist(
	...
	'$date', '$Id' , '$leftlist','$rightlist','$anserlist','5013'
)";
