...
# #         file_name = "./db/thumbnail/"+youtube_id+"_maxresdefault.jpg"
...
# #         get_data.append(item["snippet"]["publishedAt"].replace('T', ' ').replace('Z', ''))
...
# #         get_data.append(round(int(item["statistics"]["likeCount"])/(int(item["statistics"]["likeCount"])+int(item["statistics"]["dislikeCount"])),2))
...
# #         data_csv.close()
# #         p=open("index2.html","r",encoding="utf-8")
# #         data = p.read()
# #         row = data.split("\n")
# #         p.close()
# #         row.pop()
...
# #         row.append("<td><img src='"+str(file_name)+"' width='120' height='90'></td>\n")
# #         row.append("<td>"+str(dt_now.strftime("%Y-%m-%d %H:%M:%S"))+"</td>\n")
# #         row.append("<td>"+str(item["snippet"]["title"])+"</td>\n")
# #         row.append("<td>"+str(youtube_id)+"</td>\n")
# #         row.append("<td>"+str(item["snippet"]["publishedAt"].replace('T', ' ').replace('Z', ''))+"</td>\n")
# #         row.append("<td>"+str(item["snippet"]["channelTitle"])+"</td>\n")
# #         row.append("<td>"+str(item["statistics"]["viewCount"])+"</td>\n")
# #         row.append("<td>"+str(item["statistics"]["likeCount"])+"</td>\n")
# #         row.append("<td>"+str(item["statistics"]["dislikeCount"])+"</td>\n")
# #         row.append("<td>"+str(round(int(item["statistics"]["likeCount"])/(int(item["statistics"]["likeCount"])+int(item["statistics"]["dislikeCount"])),2))+"</td>\n")
# #         row.append("<td>"+str(item["statistics"]["commentCount"])+"</td>\n</tr>\n")

# #         row.append("</tbody>\n")
# #         row.append("</table>\n")
# #         row.append("</body>\n")
# #         row.append("</html>\n")
# #         p = open("index2.html","w",encoding="utf-8")
...
# # # data = r.json()
# # # print(json.dumps(data["items"][0]["snippet"]["resourceId"]["videoId"], indent=4))
...
# #単語データを読み込む
# with open("./txt/word_only/"+target_id + ".txt", 'r', encoding='utf8') as f:
#     CONTENT = f.read()
# stopwords = [u"いる",u"なら",u"する",u"ある",u"ない",u"こと",u"なっ",u"でき",u"できる",u"これ",u"もの",u"よう",u"なる",u"はず",u"なり",u"それ",u"れる",u"られる",u"ここ",u"ため",u""]
# word_cloud = WordCloud(max_font_size=200, width=1280, height=720, font_path="./LightNovelPOPv2.otf", stopwords=set(stopwords)).generate(CONTENT)
# word_cloud.to_file("./html_view/plt/wordcloud/"+youtube_id+".png")


# #ファイルに保存
# # wordcloud.to_file("保存するファイル名")
...
#             get_data.append(item["statistics"]["likeCount"])
#             get_data.append(item["statistics"]["dislikeCount"])
#             get_data.append(round((int(item["statistics"]["likeCount"])/(int(item["statistics"]["likeCount"])+int(item["statistics"]["dislikeCount"])))*100,2))
#             get_data.append(item["statistics"]["commentCount"])
#             # print(get_data)
#             data_csv = open("./db/data.csv","a",encoding="utf-8")
#             writer  = csv.writer(data_csv, lineterminator='\n')
...
#             # row.append('<p><a href="./html/'+youtube_id+'.html">'+youtube_id+'</p>\n')
#             row.append("<tr>\n<td>"+str(line_count)+"</td>\n")
#             row.append("<td><img src='"+str(file_name)+"' width='128' height='72'></td>\n")
#             row.append("<td>"+str(dt_now.strftime("%Y-%m-%d %H:%M:%S"))+"</td>\n")
#             row.append("<td>"+str(item["snippet"]["title"])+"</td>\n")
#             row.append("<td><a href='https://www.youtube.com/watch?v="+str(youtube_id)+"'>"+str(youtube_id)+"</a></td>\n")
#             row.append("<td>"+str(item["snippet"]["publishedAt"].replace('T', ' ').replace('Z', ''))+"</td>\n")
#             row.append("<td><a href='https://www.youtube.com/channel/"+str(item["snippet"]["channelId"])+"'>"+str(item["snippet"]["channelTitle"])+"</a></td>\n")
#             row.append("<td>"+str(item["statistics"]["viewCount"])+"</td>\n")
#             row.append("<td>"+str(count_row)+"</td>\n")
#             row.append("<td>"+str(count_w)+"</td>\n")
#             row.append("<td><a href='./html/"+youtube_id+".html'>"+ str(count_ij)+"</a></td>\n")
#             row.append("<td>"+str(item["statistics"]["likeCount"])+"</td>\n")
#             row.append("<td>"+str(item["statistics"]["dislikeCount"])+"</td>\n")
#             row.append("<td>"+str(round((int(item["statistics"]["likeCount"])/(int(item["statistics"]["likeCount"])+int(item["statistics"]["dislikeCount"])))*100,2))+"</td>\n")
#             row.append("<td>"+str(item["statistics"]["commentCount"])+"</td>\n</tr>\n")
...
# html += '<img src="../thumbnail/'+youtube_id+'_maxresdefault.jpg" width="1280" height="720">'

# html += '<p>15秒あたりのコメント数 </p>\n'
# html += '<img src="../plt/interjection/'+youtube_id+'.png">\n'
# html += '<p>MACDによるバースト検知 </p>\n'
# html += '<img src="../plt/macd/'+youtube_id+'.png">\n'

# html += '<h3>草、wのチャット数ランキング</h3>\n'
...
#     html += '<td><a href="https://www.youtube.com/embed/'+youtube_id+'?start='+str(s-30)+'" target="blank">'+str(s-30)+"</a></td>\n"
#     html +='<td>'+str(_)+'</td>\n'
#     # str(s-30)+' +-30s】</h4>\n'
...
# html += "<h3>チャット内単語出現ランキングTop20</h3>"
...
# html += '<img src="../plt/wordcloud/'+youtube_id+'.png">'
